...
class Base_Task(gym.Env):
    ...
    def _init_task_env_(self, table_xy_bias=[0, 0], table_height_bias=0, **kwags):
        ...
        if self.save_demo:
            self.demo_traj, self.obs_dict, self.actions_traj, self.demo_step, self.demo_check = [], {}, [], 0, 0
    ...
    def check_set_success(self, step):
        pass
    ...
    def get_obs(self):
        ...
        if self.data_type.get("endpose", False):
            ...
            left_endpose = trans_endpose_quat2rpy(self.robot.get_left_ee_pose(), norm_gripper_val[0])
            right_endpose = trans_endpose_quat2rpy(self.robot.get_right_ee_pose(), norm_gripper_val[1])
    ...
    def _take_picture(self, terminate = False):  # save data
        if not self.save_data and not self.save_demo:
            return
        
        if self.save_demo:
            if self.demo_step == 0 and not terminate:
                ...
                self.demo_step += 1
            elif self.demo_step <= 50 and not terminate:
                pkl_dic = self.get_demo_state()
                self.actions_traj.append(pkl_dic["joint_action"]["vector"])
                self.demo_step += 1
            else:
                get_reward = self.demo_check
                if self.demo_check: self.demo_check = 0
                if self.demo_step > 1:
                    self.demo_traj.append((self.obs_dict, np.array(self.actions_traj), get_reward))
