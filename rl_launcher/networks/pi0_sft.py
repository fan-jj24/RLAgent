...
class Pi0(_model.BaseModel):
    ...
    def __init__(self, config: Pi0Config, rngs: nnx.Rngs):
        ...
        self.affine_proj_in = nnx.Linear(in_features=2048, out_features=32, kernel_init=nnx.initializers.zeros, rngs=rngs)
        self.affine_proj_out = nnx.Linear(in_features=32, out_features=2, kernel_init=nnx.initializers.zeros, rngs=rngs)
    ...
    def embed_prefix(
        ...
        if obs.tokenized_prompt is not None:
            ...
            prompt_embedding = tokenized_inputs.mean(axis=1)
            affine_params = self.affine_proj_in(prompt_embedding)
            affine_params = nnx.swish(affine_params)
            affine_params = self.affine_proj_out(affine_params)
            scale = affine_params[:, 0:1, None]
            shift = affine_params[:, 1:2, None]
            img_tokens = jnp.concatenate(tokens, axis=1)
            img_tokens = img_tokens * (1 + scale) + shift
            tokens = []
