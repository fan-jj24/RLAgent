...
from serl_launcher.utils.transforms import Normalize, Unnormalize, make_bool_mask, pad_to_dim
from serl_launcher.utils import image_tools, normalize
from serl_launcher.networks import tokenizer
...
class rl_config:
    def __init__(self):
        self.action_space = gym.spaces.Box(
            -np.inf, np.inf, shape=(3,32), dtype=np.float32  
        )
        ...
        self.per_update_steps = 4
        self.iters = 6
        self.ACTOR_RESTART_INTERVAL = 10000
        ...
        self.kl_div_coef = 0.1
        self.target_kl_div = 0.4
        self.temperature_coef = 1000
...
class RoboTwinEnv():
    ...
    def reset(self, task_name = None, mode = None, save_video = False, now_seed = None, max_seed = 100000, strict = False, execute = False, instruction = None):
        ...
        while True:
            ...
            if (not execute) and (mode != "demo" or strict) and (not s1 or not s2):
                now_seed += 1
                continue
            else:
                if mode == "demo":
                    self.args["save_demo"] = True
                    self.args["save_freq"] = 9
                    if s1 and s2:
    ...
    def step(self, actions):
        ...
        sub_reward = self.task.check_set_success()
        # done = True if self.task.eval_success or self.task.take_action_cnt >= self.task.step_lim else False
        done = True if self.task.take_action_cnt >= self.task.step_lim else False
        sub_reward = 1.0 if self.task.eval_success else sub_reward
        return next_observation, done, {"progress": sub_reward}
    ...
    def input(self, observation):
        ...
        observation["image"] = {k: image_tools.resize_with_pad(v) for k, v in observation["image"].items()}
